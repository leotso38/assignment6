...
# Notes: Tests CalculatorConfig env overrides, defaults, path resolution, and validation.
...
# Preseed environment for deterministic behavior in most tests
...
def clear_env_vars(*args):
    """Helper: unset specific env vars."""
    for var in args:
...
def test_directory_properties():
    # Derived dirs default from base_dir when env overrides absent
...
def test_file_properties():
    # Derived files default from base_dir when env overrides absent
...
def test_default_fallbacks():
    # Clear key env vars → class defaults should apply
    clear_env_vars(
...
def test_get_project_root():
    # get_project_root should point to repo root (contains app/)
